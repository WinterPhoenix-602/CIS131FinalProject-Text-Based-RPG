...
class Enemy:
    ...
    def attack(self, target=Player()):
        ...
        if attempt <= self._accuracy and (self._damage - (target.defense + target._equippedShield.stats["Defense"])) > 0:
            target.modify_health(-self._damage + target.defense)
            print(tabulate(
                [[f"{self._name} hits you and deals {target.defense + target._equippedShield.stats['Defense']} damage!"]], tablefmt="fancy_outline"))
        elif attempt <= self._accuracy:
            print(tabulate(
                [[f"{self._name} attacks, and you deflect the blow with your {target.equippedShield.name}!"]], tablefmt="fancy_outline"))
        else:
            print(tabulate(
